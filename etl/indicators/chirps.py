...
class CHIRPS(Indicator):
    ...
    # ---- 15-day rainfall forecast (CHIRPS-GEFS v3) ----
    #
    # UCSB's bias-corrected GEFSv12 forecast, quantile-matched to the CHIRPS
    # distribution. Streamed directly from data.chc.ucsb.edu via /vsicurl/ —
    # see etl/chirps_gefs.py. No GEE quota, no permanent file storage.

    def fetch_forecast(self, issuance_date: date | None = None) -> pd.DataFrame:
        """Pull the latest CHIRPS-GEFS issuance and return 3 forward 5-day pentads.
        ...
        If `issuance_date` is None or that day's files aren't published yet,
        walks back up to 3 days to find the latest fully-published issuance.
        """
        from .. import chirps_gefs

        target = issuance_date
        if target is None or not chirps_gefs.issuance_available(target):
            target = chirps_gefs.latest_available_issuance()
            if target is None:
                return pd.DataFrame()

        df = chirps_gefs.fetch_pentad_totals(target)
        if df.empty:
            return df
